...
- **不具合1: 解消済み**（コミット `223ca2f` 「エラーの修正」で修正。
  `decreased_items` が空の場合も `processed_session_ids` に追加するよう修正され、
  無限リトライは発生しなくなった）
- **不具合2: 緩和済み・根本原因は残存**（同コミットで、処理済みセッションに対する
  動画コピー/プレビュー生成をスキップするよう順序変更されたため、同じエラーが
  繰り返される頻度は大幅に減った。ただし新規セッションを初めて処理する瞬間の
  `permission denied` 自体は、`controller.py`(root実行)と`web_admin`(一般ユーザー実行)
  の権限不一致が原因であり未解消。運用面での回避策は本ファイル該当セクション参照）

以下は発見時の記録（原因調査の経緯として残す）。

---

## 不具合1（解消済み）: エラー終了したセッションが無限に再試行され、ログにエラーが出続ける
